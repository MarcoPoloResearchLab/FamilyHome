...
output=build/tests/piano-audio
...
if [[ ! -f "$keystore" ]]; then
  keytool -genkeypair -noprompt -keystore "$keystore" -storepass android -keypass android \
    -alias test -keyalg RSA -validity 3650 -dname 'CN=Piano Audio Test' >/dev/null
fi
cleanup() {
  "$adb" uninstall com.mprlab.portal.pianotest >/dev/null 2>&1 || true
  "$adb" uninstall com.mprlab.portal >/dev/null 2>&1 || true
}
trap cleanup EXIT
FAMILYHOME_SERVICE_BASE_URL=https://familyhome.invalid \
FAMILYHOME_DEVICE_TOKEN=familyhome-piano-test-token-000000 \
PORTAL_KEYSTORE="$keystore" PORTAL_KEYSTORE_PASSWORD=android PORTAL_KEY_PASSWORD=android \
APK_BASENAME=piano-app ./build.sh "$output/app"
"$tools_dir/aapt2" link -I "$android_jar" --manifest tests/piano-audio/AndroidManifest.xml -o "$output/test.apk"
javac -source 8 -target 8 -classpath "$android_jar" -d "$output/classes" tests/piano-audio/PianoAudioTest.java
"$tools_dir/d8" --lib "$android_jar" --min-api 28 --output "$output/dex" "$output/classes/com/mprlab/portal/pianotest/PianoAudioTest.class"
...
"$adb" install "$output/app/piano-app.apk"
"$adb" install "$output/test-signed.apk"
result="$("$adb" shell am instrument -w com.mprlab.portal.pianotest/.PianoAudioTest)"
printf '%s\n' "$result"
[[ "$result" == *'Piano audio passed:'* ]]
