...
if [[ ! -f "$keystore" ]]; then
  keytool -genkeypair -noprompt -keystore "$keystore" -storepass android -keypass android \
    -alias test -keyalg RSA -validity 3650 -dname 'CN=Guitar Test' >/dev/null
fi
cleanup() {
  "$adb" uninstall com.mprlab.portal.guitartest >/dev/null 2>&1 || true
  "$adb" uninstall com.mprlab.portal >/dev/null 2>&1 || true
}
trap cleanup EXIT
FAMILYHOME_SERVICE_BASE_URL=https://familyhome.invalid \
FAMILYHOME_DEVICE_TOKEN=familyhome-guitar-test-token-000000 \
PORTAL_KEYSTORE="$keystore" PORTAL_KEYSTORE_PASSWORD=android PORTAL_KEY_PASSWORD=android \
APK_BASENAME=guitar-app ./build.sh "$output/app"
"$tools_dir/aapt2" link -I "$android_jar" --manifest tests/guitar/AndroidManifest.xml -o "$output/test.apk"
javac -source 8 -target 8 -classpath "$android_jar" -d "$output/classes" tests/guitar/GuitarTest.java
"$tools_dir/d8" --lib "$android_jar" --min-api 28 --output "$output/dex" "$output/classes/com/mprlab/portal/guitartest/GuitarTest.class"
...
"$adb" install "$output/app/guitar-app.apk"
"$adb" install "$output/test-signed.apk"
result="$("$adb" shell am instrument -w com.mprlab.portal.guitartest/.GuitarTest)"
printf '%s\n' "$result"
[[ "$result" == *'Guitar passed:'* ]]

"$adb" pull /sdcard/Android/data/com.mprlab.portal/files/guitar.png "$output/guitar.png"
