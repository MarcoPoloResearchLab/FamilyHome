...
"$tools_dir/aapt2" link -I "$android_jar" --manifest "$source_dir/AndroidManifest.xml" --java "$output_dir" \
  --min-sdk-version 28 --target-sdk-version 28 --version-code 13 --version-name 0.9.0 \
...
"$tools_dir/zipalign" -f 4 "$output_dir/portal-with-dex.apk" "$output_dir/Children-Portal-v0.9.0-aligned.apk"
...
if [[ -n "${PORTAL_KEYSTORE:-}" ]]; then
  ...
  "$tools_dir/apksigner" sign --ks "$PORTAL_KEYSTORE" --ks-pass "pass:$PORTAL_KEYSTORE_PASSWORD" \
    --key-pass "pass:$PORTAL_KEY_PASSWORD" --out "$output_dir/Children-Portal-v0.9.0.apk" \
    "$output_dir/Children-Portal-v0.9.0-aligned.apk"
  "$tools_dir/apksigner" verify --verbose "$output_dir/Children-Portal-v0.9.0.apk"
fi
